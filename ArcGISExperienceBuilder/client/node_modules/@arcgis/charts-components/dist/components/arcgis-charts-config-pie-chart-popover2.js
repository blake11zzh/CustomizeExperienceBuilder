...
import { M as Mn, w as ta, y as eu, A as Tl, C as Cc, I as Ic } from './index2.js';
import { e as eh } from './index3.js';
import { P as PieChartDataPopoverKinds, b as RESTFieldType } from './interfaces.js';
import { e as emitNumericFieldsChange, p as pieAggregationList, U as UIDefaults, i as itemMatchesSelectedContent } from './chart-ui-utils.js';
...
const ArcgisChartsConfigPieChartPopover = /*@__PURE__*/ proxyCustomElement(class ArcgisChartsConfigPieChartPopover extends HTMLElement {
    constructor() {
        super();
        this.__registerHost();
        this.__attachShadow();
        this.arcgisChartsConfigPieChartDataPopoverChange = createEvent(this, "arcgisChartsConfigPieChartDataPopoverChange", 7);
         ...
         */
        this.closePopover = () => {
            if (this.contentKind === PieChartDataPopoverKinds.numericFields && this.pickListHasChanged) {
    ...
    async componentWillRender() {
        const listPromises = pieAggregationList.map(async (item) => await ta(item));
    ...
    buildAggregationPickListItems() {
        return pieAggregationList.map((item, index) => (h("calcite-list-item", { label: this.aggregationLabels[index], key: item, value: item, selected: itemMatchesSelectedContent(item, this.selectedContent) })));
    }
    /**
     * @category Render UX
     */
    renderCategory() {
        const items = this.buildPickListItems(PieChartDataPopoverKinds.category);
     ...
     */
    renderNumericFields() {
        const items = this.buildPickListItems(PieChartDataPopoverKinds.numericFields);
    ...
    renderSortOrder() {
        ...
        return (h("calcite-list", { label: "", selectionMode: "single", selectionAppearance: "border", filterEnabled: false, onCalciteListChange: this.onDataContentTypeChange }, items));
    }
    /**
     * @category Render UX
     */
    buildSortOrder() {
        const items = Object.keys(eh);
        // remove `customSort` from the list
        items.splice(items.indexOf(eh.customSort), 1);
        return items.map((sortOrder) => {
            ...
            return (h("calcite-list-item", { selected: this.selectedContent === sortOrder, key: sortOrder, label: this.strings[sortOrder], value: eh[sortOrder] }, h("calcite-icon", { slot: "content-end", icon: icon1 }), h("calcite-icon", { class: iconClass2, slot: "content-end", icon: icon2 })));
        });
    }
    /**
     * @param field
    ...
    render() {
        let footerButton;
        if (this.contentKind === PieChartDataPopoverKinds.numericFields) {
            footerButton = (h("div", { key: 'c724640b31c36da7a8d04c49ff9f02335743cef4', slot: "footer" }, h("calcite-fab", { key: '4bc04304adcf2bcf158cd73cb0fe18c45826476a', class: HTMLClasses.fab, appearance: "outline-fill", kind: "neutral", icon: "", label: this.strings.selectionDone, scale: "s", "text-enabled": true, text: this.strings.selectionDone, onClick: this.onNumericFieldSelectionDone })));
        }
        return (h(Host, { key: '6803285fafc88cdc9f5e3a6d56adda5340286ac0', class: HTMLClasses.jsAppFlyout }, h("calcite-popover", { key: 'c42b35f11af5343ec04913c6a68cc9acffc2e5e6', class: HTMLClasses.name, referenceElement: this.referenceElement, placement: this.placement, onCalcitePopoverClose: this.closePopover, offsetDistance: this.offsetDistance, open: this.open, label: "", ref: (e) => {
                this.popoverElement = e;
            } }, h("calcite-panel", { key: '6c1f43eb0cd5bfece088b12da0c4e33f88c14f3c', class: HTMLClasses.panel, closable: true, closed: !this.open, onCalcitePanelClose: this.closePopover }, h("div", { key: '44ffa52698271f705ef9f3588789499f77e5e534', slot: "header-content", class: HTMLClasses.header }, this.headingTitle ?? ""), this.renderPopoverInfo(), footerButton))));
    }
    ...
    static get style() { return ArcgisChartsConfigPieChartPopoverStyle0; }
}, [1, "arcgis-charts-config-pie-chart-popover", {
...
function defineCustomElement() {
    ...
    const components = ["arcgis-charts-config-pie-chart-popover"];
    components.forEach(tagName => { switch (tagName) {
        case "arcgis-charts-config-pie-chart-popover":
            if (!customElements.get(tagName)) {
                customElements.define(tagName, ArcgisChartsConfigPieChartPopover);
            }
            break;
    } });
}
...
export { ArcgisChartsConfigPieChartPopover as A, defineCustomElement as d };
