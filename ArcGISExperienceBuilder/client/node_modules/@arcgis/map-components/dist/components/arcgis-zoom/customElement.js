...
import { LitElement as h, createEvent as z, noShadowRoot as p, safeClassMap as s, nothing as a } from "@arcgis/lumina";
...
}, f = y`@layer{arcgis-zoom,.arcgis-zoom{display:flex;flex-flow:column nowrap}.arcgis-zoom__zoom-out{border-block-start:solid 1px var(--calcite-color-border-1)}.arcgis-zoom--horizontal{flex-flow:row-reverse nowrap}.arcgis-zoom--horizontal .arcgis-zoom__zoom-out{border-block-start:none;border-inline-end:solid 1px var(--calcite-color-border-1)}}`, w = b(d);
class v extends h {
  constructor() {
    super(...arguments), this.messages = g({}), this.viewModel = w(this), this.canZoomIn = this.viewModel.canZoomIn, this.canZoomOut = this.viewModel.canZoomOut, this.autoDestroyDisabled = !1, this.icon = "magnifying-glass-plus", this.layout = "vertical", this.position = "top-left", this.state = this.viewModel.state, this.arcgisPropertyChange = u()("state"), this.arcgisReady = z();
  }
  static {
  ...
  render() {
    const r = {
      [o.horizontalLayout]: this.layout === "horizontal"
    }, { canZoomIn: n, canZoomOut: l } = this.viewModel, { zoomIn: e, zoomOut: i } = this.messages;
    return m`<div class=${s(t(o.base, r))}><calcite-button class=${s(t(o.button, o.zoomIn))} .disabled=${!n} icon-start=plus kind=neutral .label=${e} @click=${this.zoomIn} title=${e ?? a}></calcite-button><calcite-button class=${s(t(o.button, o.zoomOut))} .disabled=${!l} icon-start=minus kind=neutral .label=${i} @click=${this.zoomOut} title=${i ?? a}></calcite-button></div>`;
  }
}
c("arcgis-zoom", v);
export {
