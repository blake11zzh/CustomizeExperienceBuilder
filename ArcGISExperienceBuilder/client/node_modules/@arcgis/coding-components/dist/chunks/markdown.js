...
async function ze(l) {
  if (!l)
    return;
  const t = [...l.querySelectorAll("code.language-arcade")];
  await Promise.all(
    t.map(async (n) => await ie(n, n.textContent))
  );
}
async function ie(l, t) {
  !l || l.tagName !== "CODE" || (l.innerHTML = await ne(t ?? ""));
}
function Se() {
...
class S {
  constructor(t) {
    this.tokens = [], this.tokens.links = /* @__PURE__ */ Object.create(null), this.options = t || A, this.options.tokenizer = this.options.tokenizer || new q(), this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = {
      inLink: !1,
      inRawBlock: !1,
      top: !0
    };
...
class we {
  ...
  use(...t) {
    ...
    return t.forEach((e) => {
      ...
      if (e.hooks) {
        const s = this.defaults.hooks || new D();
        for (const r in e.hooks) {
          const a = s[r];
          D.passThroughHooks.has(r) ? s[r] = (o) => {
