import { g as B, c as l } from "../../chunks/runtime.js";
import { i as s } from "../../chunks/monaco-importer.js";
import { Deferred as w, generateGuid as E, observeAncestorsMutation as Q, getElementTheme as m } from "@arcgis/components-utils";
import { LitElement as h, createEvent as r, noShadowRoot as M } from "@arcgis/lumina";
import { g as x, s as p } from "../../chunks/arcade-service-accessors.js";
import { languages as c, Range as I, Uri as a, KeyMod as d, KeyCode as g } from "monaco-editor";
...
import { a as t } from "../../chunks/sql-expr-defaults.js";
...
async function D() {
  return await import("../../chunks/sql-expr-mode.js");
}
c.register({
  id: t.languageId,
...
function u() {
  s().then((e) => {
    ["light", "dark"].forEach((A) => {
      const i = A === "light" ? "vs" : "vs-dark", o = F[`${A}-theme`];
      e.defineTheme(i, {
        base: i,
        inherit: !0,
        rules: [
...
class z extends h {
  constructor() {
    super(...arguments), this._componentReadyDefer = new w(), this._disposables = [], this._monacoEditor = b(s), this.modelId = E(), this.value = "", this.arcgisSelectionChange = r({ bubbles: !1 }), this.arcgisValueChange = r({ bubbles: !1 });
  }
  static {
  ...
  async insertText(A) {
    if (await this._componentReadyDefer.promise, !this._editorInstance || !this._editorInstance.getModel())
      return;
    const o = this._editorInstance.getSelection() ?? new I(1, 1, 1, 1);
    this._editorInstance.executeEdits("", [{ range: o, text: A ?? "", forceMoveMarkers: !0 }]);
  }
  /**
  ...
  loaded() {
    ...
    const A = this._monacoEditor.createModel(this.value, this.language, this.getUri());
    this._editorInstance = this._monacoEditor.create(this.el, {
      ...k,
      ...this.editorOptions ?? {},
      model: A,
  ...
  _updateModelValue() {
    if (!this._editorInstance)
      return;
    const A = this._editorInstance.getModel();
    if (!A || A.getValue() === this.value)
      return;
    const i = A.getFullModelRange();
    this._editorInstance.executeEdits("", [{ range: i, text: this.value, forceMoveMarkers: !0 }]), this._editorInstance.pushUndoStop();
  }
  _updateModel() {
    if (!this._editorInstance)
      return;
    const A = this._editorInstance;
    A.getModel()?.dispose(), A.setModel(this._monacoEditor.createModel(this.value, this.language, this.getUri()));
  }
  getUri() {
  ...
  addCommonEditorActions() {
    const A = this._editorInstance?.getAction("editor.action.fontZoomIn"), i = this._editorInstance?.getAction("editor.action.fontZoomOut");
    this._editorInstance?.addAction({
      ...
      keybindings: [d.CtrlCmd | g.Equal],
      contextMenuGroupId: "zooming",
      contextMenuOrder: 1,
      // Method that will be executed when the action is triggered.
      // @param editor The editor instance is passed in as a convenience
      run: async () => await A?.run()
    }), this._editorInstance?.addAction({
      id: "editor.action.fontZoomOut",
      label: "Editor Font Zoom Out",
      keybindings: [d.CtrlCmd | g.Minus],
      contextMenuGroupId: "zooming",
      contextMenuOrder: 2,
      run: async () => await i?.run()
    });
  }
}
l("arcgis-code-editor", z);
export {
